var OrderView = function () {
	...
	this.saveOrderToDB = function(){
		...
		that.oOrder.received = that.total;
		that.oOrder.ship = that.totalShip;
		that.oOrder.discountCustomer = that.moneyDiscountCustomer;
		that.oOrder.discountVoucher = that.moneyDiscountVoucher;
		...
		that.oOrder.customerRanking = that.oCustomer.ranking;
		...
		for (let i = 0;i<that.listProduct.length;i++){
			...
			oOderProduct.realPrice = item.price;
		...
		for (let i = 0;i<that.listService.length;i++){
			...
			oOrderService.discount = item.price*0.01*item.item.discount;
